...
import SorteoSelectorPage from './pages/SorteoSelectorPage';
...
export default function App() {
  const { authenticated, sorteoSelected, sorteoActivo, changeSorteo } = useAuth();

  // En la ruta pública /info no hay sesión: el sorteo viene del QR (?sorteo=ID)
  const isInfoRoute = typeof window !== 'undefined' && window.location.pathname === '/info';
  const qrSorteoId = isInfoRoute
    ? Number(new URLSearchParams(window.location.search).get('sorteo')) || null
    : null;

  const sorteoId = isInfoRoute ? qrSorteoId : (sorteoActivo?.id ?? null);
  const {
    ...
    <div className="app">
      <Routes>
        ...
        <Route path="/info" element={
          <main className="main">
            <PublicMobile
              ...
              estadoLoaded={estadoLoaded} sorteoId={sorteoId}
            />
          </main>
        } />
        ...
        <Route path="/login" element={
          authenticated
            ? (sorteoSelected ? <Navigate to="/" replace /> : <Navigate to="/sorteo" replace />)
            : <LoginPage />
        } />

        {/* Selección de sorteo — solo para autenticados sin sorteo elegido */}
        ...
        <Route path="/importar" element={
          <ProtectedRoute>
            <AuthShell {...shellProps}>
              {error && <ErrorBanner error={error} />}
              <main className="main">
                <ImportarPage onNavigate={(path) => window.location.href = path} />
              </main>
            </AuthShell>
          </ProtectedRoute>
        } />
