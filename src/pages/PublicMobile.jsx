...
// ── Main component ───────────────────────────────────────────────────────
export default function PublicMobile({ lotes, items, resultados, loteActivo, lastSyncAt, syncing, estadoLoaded, sorteoId }) {
  const [tab, setTab] = useState('resumen');
  const { sorteoActivo } = useAuth();
  const [sorteoRemoto, setSorteoRemoto] = useState(null);

  // En /info no hay sesión (sorteoActivo es null): buscamos el nombre
  // del sorteo directamente por el id que viene del QR.
  useEffect(() => {
    if (sorteoActivo || !sorteoId) return;
    sorteosDb.getSorteoById(sorteoId).then(({ data }) => {
      if (data) setSorteoRemoto(data);
    });
  }, [sorteoActivo, sorteoId]);

  const sorteoNombre = sorteoActivo?.nombre || sorteoRemoto?.nombre || 'DIE-2026';
